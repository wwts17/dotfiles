...
- `commands/feature.md` — `/feature`: the fixed sequence for delivering a feature. Order of calls: clarify with the user → read affected code → `skills/tech-doc` (Plan or Design) → wait for confirmation, Decisions needed answered one by one → implement with `skills/coding`: a Plan one verified step at a time, a Design one stage at a time with `commands/quality-review` after each stage — one `first` round, then at most two `recheck` rounds — and the next stage gated on Must fix being empty → a final cross-stage review (`/code-review` at effort `medium` plus the reviewer's Problems introduced and Internal consistency dimensions) → final report.
- `commands/quality-review.md` — `/quality-review`: review finished changes without editing them. Runs as `first` or `recheck`. `first` is the full sequence: determine the scope (argument, or the current branch against `main` with uncommitted work included) → list the changed files, one sentence each → built-in `/code-review` at effort `medium` → read the callers and existing tests of every changed function → `agents/reviewer` in `first` mode. `recheck` skips the file listing and the caller reading, runs `/code-review` over the fixed files only, and calls `agents/reviewer` in `recheck` mode with the previous report's path so it can check each earlier Reject. Both write the full `/code-review` and reviewer output to `~/.claude/reviews/<repo>/<branch>-<round>.md` and put only that path in the chat; the chat gets the three-section report — must-fix (reviewer Rejects only), fix-next, and before-going-live split into what the user decides and what the user verifies.
