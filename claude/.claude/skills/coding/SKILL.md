---
name: coding
description: Engineering principles for all code, with worked ❌/✅ examples. Use when implementing, reviewing, or refactoring code.
---
...
- Decide who owns each piece of mutable state. When several execution flows touch it, hand ownership through a queue or channel instead of guarding shared state with locks.
...
- Write the direct implementation first. Fancy algorithms are buggier, have big constants, and n is usually small. When something turns out to be slow, use the performance skill.
...
- A value is usable the moment it is constructed. No separate `init()` call before it works.
...
- Usable on construction. ❌ `new User()` then `.init()` ✅ the constructor leaves it ready
