...
# orbstack: docker / docker-compose / kubectl live in ~/.orbstack/bin, installed by
# the OrbStack app rather than brew. fpath+= after compinit is fine — zsh autoloads
# those completions lazily (same reason the compinit-before-antidote note above holds).
# Kept here instead of the installer's ~/.zprofile line so non-login shells get it too.
